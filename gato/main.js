...
const boardContainer = d.querySelector('#board')
const playerDiv = d.querySelector('#player')
...
function checkIfCanWin(){
  ...
  const res = [s1,s2,s2,s3,s4,s5,s6,s7,s8].filter((line) =>{
...
function checkIfWinner(){
    ...
    const res = [s1,s2,s2,s3,s4,s5,s6,s7,s8].filter((line) =>{
    return (
      line[0] + line[1] + line[2] === 'XXX' || line[0] + line[1] + line[2] === 'OOO'
    )
  })
  if (res.length > 0) {
    debugger
